...
export const save_request_results = async (
  request_id, database, table_name, result, index
) => {
...
const get_select_results_dir = (request_id, database, table_name) => {
  return `${get_table_dir(database, table_name)}/requests/${request_id}/results`;
}
...
export const process_select_from_select = async (
  ...
  await execute_select_on_select(
    request_id, address, from_request_id, state, csk, commit,
  );
...
const execute_select_on_select = async (
  ...
  const executions_dir = get_select_executions_dir(request_id, address, from_request_id);
...
export const parse_results_select_from_commit = async (
  request_id,
  table,
) => {
  const address = global.context.account.address().to_string();
...
export const throw_verify_select_from_commit = async (
  ...
  const expected_execution_amount = (
  ...
  if (execution_indexes.length !== expected_execution_amount) {
    throw new Error(
      `Request '${request_id}' has ${execution_indexes.length} executions, but ${expected_execution_amount} were expected.`
    );
  }
