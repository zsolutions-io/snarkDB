...
const COPY_LIST = ["utils"];


export async function load_cached_program_source(
  program_id
) {
  const copy_from_program_dir_if_not_exist = COPY_LIST.includes(program_id);
...
export async function verify_execution_no_cache(
  ...
  try {
    const program = programManager.createProgramFromSource(program_code);
