...
export async function verify_execution(
  ...
  try {
    ...
    console.log("program", program.getImports());
...
export async function load_or_generate_and_save_program_keys(
  program_source,
  function_name,
  inputs,
  proving_private_key,
  ...
  if (proving_path_exists && verifying_path_exists) {
    return await load_program_keys(function_name, program_dir_path);
  }
  else {
    return await generate_and_save_program_keys(
      program_source,
      function_name,
      inputs,
      proving_private_key,
